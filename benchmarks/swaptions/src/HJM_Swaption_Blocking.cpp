    ...
    long iRndSeed, long lTrials, int BLOCKSIZE, int tid)
{
	...
	counter++;
	printf("counter in HJM_Swaption_Blocking: %d\n", counter);
	...
	dSimSwaptionStdError = sqrt((dSumSquareSimSwaptionPrice - dSumSimSwaptionPrice * dSumSimSwaptionPrice / lTrials) / (lTrials - 1.0))
	    / sqrt((FTYPE) lTrials);
