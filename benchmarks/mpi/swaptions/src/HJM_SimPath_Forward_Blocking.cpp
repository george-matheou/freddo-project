...
//MultipleDThread* t2;
//
//void dthread_t2(ContextArg context)
//{
//	printf("Context: %d\n", context);
//
//	// For waiting
...
//This function computes and stores an HJM Path for given inputs
	...
	/*counter=0;
	 ...
	 t2 = new DThread(dthread_t2, Nesting::ONE, 1);
	 t2->initUpdate(CREATE_N1(0), CREATE_N1(BLOCKSIZE-1));

	 // Wait all instances (because we don't want to call ddm::run again)
	...
	/**
	 * Parallelize this with freddo. Currently is not possible becasue we have to call run().
	 * The run function is called in main. So if we wait on counter the run will stop executing because is blocked
	 * by the while(counter != BLOCKSIZE-1).
	 */
