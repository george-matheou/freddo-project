...
void initializeData(int nodes_num, int blks, int bsize) {
	...
	try {
		...
		tmp_A = new DATA_TYPE[size];
		Alin = new DATA_TYPE[size];
		A = new DATA_TYPE**[blks];

		for (i = 0; i < blks; ++i) {
			A[i] = new DATA_TYPE*[blks];
		}
...
AddrID gasA; // The GAS_ID of matrix A
...
int main(int argc, char* argv[]) {
	...
	printf("LU benchmark has been started and starts initializations| Matrix Size: %d and Block Size: %d\n", matrixSize, blockSize);
...
printf("Double precision enabled\n");
	...
	printf("Program: LU decomposition, Port %d, Matrix Size: %d, BlockSize: %d Blocks: %d, Run Serial: %d\n", atoi(argv[1]), matrixSize, blockSize,
			blocks, atoi(argv[4]));
...
//#define TEST_DYNAMIC_SM
	...
	ddm::finalize();
	cout << "finalizing FREDDO resources\n";
...
void execute(int matrixSize, int blockSize) {
	long blocks = matrixSize / blockSize;
	DATA_TYPE *Blin;
	DATA_TYPE *tmp_B;
	DATA_TYPE ***B;
	long int ii, jj, kk;
	...
	try {
		...
		B = new DATA_TYPE**[blocks];

		for (i = 0; i < blocks; i++) {
			B[i] = new DATA_TYPE*[blocks];
		}
	} catch (std::bad_alloc& exc) {
