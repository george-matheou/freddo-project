...
#include "CoreBlas/CoreBlasTile.hpp"
#include <lapacke.h>
...
void parallel(int portNumber, char* peerFile) {
	double start, finish;
	...
	ddm::init(peerFile, portNumber, conf);
...
int main(int argc, char* argv[]) {
	if (argc < 5) {
		printf("%s <port> <matrix size> <block size> <run serial> <peer file>\n", argv[0]);
		return 1;
	}
