...
int main(int argc, char* argv[]) {
	...
	if (argc != 7) {
		printf("arguments: <port> <MatrixSize> <BlockSize> <Run Serial> <Peers File> <Sim_Iter_Num>\nEg. program 3346 1024 32 1 peers.txt 2\n");
		exit(-1);
	} else {
		printf("Program: Matrix Multiplication, Port: %s, Matrix Size: %s, BlockSize: %s, Run Serial: %d Sim_Iter_Num:%d\n", argv[1], argv[2],
				argv[3], atoi(argv[4]), atoi(argv[6]));
	}
